...
def train(
    ...
    epoch = start_epoch
    metrics_dict = {"train": [],"validation": []}

    # log validation loss before _any_ training
    valid_loss = 0.0
    for _, valid_loader in valid_loaders.items():
        valid_loader_name = "validation"
    ...
    while epoch < max_num_epochs:
        ...
        if epoch % eval_interval == 0:
            ...
            with param_context:
                ...
                train_metrics["head"] = "train"
                logger.log(train_metrics)            
                metrics_dict["train"].append({
                    "epoch": epoch,
                    "loss": train_loss_head,
                    **train_metrics
                })
                if rank == 0:
                    valid_err_log(
                ...
                for _, valid_loader in valid_loaders.items():
                    ...
                    metrics_dict["validation"].append({
                        "epoch": epoch,
                        "loss": valid_loss_head,
                        **eval_metrics
                    })
                    if rank == 0:
                        valid_err_log(
